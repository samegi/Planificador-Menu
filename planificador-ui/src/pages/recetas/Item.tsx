...
export default function RecetaItem() {
  ...
  useEffect(() => {
    ...
    RecetasApi.obtener(recetaId)
      ...
      .catch((err) => {
        ...
        alert('No se pudo cargar la receta. Mira la consola.');
      })
      .finally(() => setLoading(false));
  }, [recetaId]);

  const updateField = (field: keyof Receta | 'ingredientes' | 'instrucciones', value: string) =>
    setReceta((prev) => (prev ? { ...prev, [field]: value } as Receta : prev));
  ...
  const guardar = async () => {
    ...
    try {
      await RecetasApi.actualizar(recetaId, {
        ...
        // estos campos son strings en tu entidad
        ...(receta as any).ingredientes !== undefined && {
          ingredientes: (receta as any).ingredientes,
        },
        ...(receta as any).instrucciones !== undefined && {
          instrucciones: (receta as any).instrucciones,
        },
      } as any);
  ...
  const eliminar = async () => {
    ...
    try {
      ...
      alert('No se pudo eliminar la receta. Mira la consola.');
    }
  };
  ...
  const instrucciones = (receta as any).instrucciones ?? '';
